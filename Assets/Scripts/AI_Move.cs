...
public class AI_Move : Car
{
    ...
    void Update()
    {
        ...
        {//Düşman algılandığında rastgele saldırır.(upGrade, 0 değilse dönmek için işlem yapıyor demektir. Bitmesini beklemeli)
            StartCoroutine(TurnArea(1, gameManager.whenAttackTurn.maxUpGrade, gameManager.whenAttackTurn.MaxAngle));
        ...
        {//arena dışına çıkması engellenir.
            StartCoroutine(TurnArea(keepRaycastValue, gameManager.whenOutAreaTurn.maxUpGrade, gameManager.whenOutAreaTurn.MaxAngle));
            return;
        }
        ...
        if (upGrade == 0 && Random.Range(0, 100) < 5)
        {
            ...
            StartCoroutine(TurnArea(Random.Range(0, 100) < 30 ? 1 : -1, gameManager.randomTurn.maxUpGrade, gameManager.randomTurn.MaxAngle));
        }
